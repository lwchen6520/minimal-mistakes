...
export default function WritingPage() {
  ...
  const filtered = useMemo(() => {
    return posts.filter((post) => {
      ...
      const matchesQuery = post.title.toLowerCase().includes(query.toLowerCase()) || post.summary.toLowerCase().includes(query.toLowerCase());
    ...
    <div className="section-shell flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
        <div className="md:col-span-2">
          <SearchBox value={query} onChange={setQuery} placeholder={language === 'zh' ? '搜尋標題或摘要' : 'Search title or summary'} />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filtered.map((post) => (
          <Card key={post.id} title={post.title} description={post.summary} tone="tech" tags={[post.category]}>
