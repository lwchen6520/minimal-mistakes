...
export default function WorkPage() {
  ...
  const filtered = useMemo(() => {
    if (activeTag === '全部') return caseStudies;
  ...
  return (
    <div className="section-shell flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-4">
        ...
        />
        <TagFilter tags={tagOptions} active={activeTag} onChange={setActiveTag} />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filtered.map((item) => (
          <Card key={item.id} title={`${item.client}`} description={item.background} tags={item.tags} tone="tech">
